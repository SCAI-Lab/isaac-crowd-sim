...
class CharacterBehavior(BehaviorScript):
    ...
    def init_character(self):
        ...
        self.navigation_manager = NavigationManager(
            str(self.prim_path), self.navmeshEnabled, self.avoidanceOn
        )
        ...
        carb.log_warn("Initialize the character")
        return True
    ...
    def get_command(self, command):
        ...
        if command[0] == "GoTo":
            ...
            try:
                custom_class = getattr(
                    importlib.import_module(module_str, package=__package__), command[0]
                )
            except (ImportError, AttributeError) as error:
                carb.log_error(
                    "Module or Class for the command do not exist. Check the command again."
                )
